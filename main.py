...
class Aircraft:
    ...
    # Cabin volume calculation (cylindrical approximation)
    def calc_cabin_volume(self, D_fuselage, L_cabin):
        return (math.pi * (D_fuselage / 2) ** 2) * L_cabin
...
def find_correct_w0(W0_initial, aircraft, mission, tank, tolerance=1e-6, max_iterations=1000):
    ...
    for iteration in range(max_iterations):
        ...
        W0_new = (aircraft.crew_weight + aircraft.payload_weight) / (1 - structual_factor * EWF - total_fuel_weight_fraction)
...
# Calculate available cabin volume
available_cabin_volume = aircraft.calc_cabin_volume(D_fuselage, L_cabin)

# Print results in a more organized table format
data_weights = [
    ["Takeoff Weight (W0)", f"{correct_W0:.2f} kg"],
    ["Empty Weight Fraction (EWF)", f"{empty_weight_fraction:.4f}"],
    ["Empty Weight (EW)", f"{correct_W0 * empty_weight_fraction:.2f} kg"],
]

data_fuel = [
    ["Fuel Mass", f"{fuel_mass:.2f} kg"],
    ["Fuel Fraction", f"{total_fuel_weight_fraction:.4f}"],
]
...
# Print fuselage dimensions and cabin volume information
data_fuselage = [
    ["Fuselage Diameter", f"{D_fuselage:.2f} m"],
    ["Fuselage Length", f"{L_fuselage:.2f} m"],
    ["Cabin Length", f"{L_cabin:.2f} m"],
    ["Required Cabin Volume", f"{required_cabin_volume_m3:.2f} m^3"],
    ["Available Cabin Volume", f"{available_cabin_volume:.2f} m^3"]
]

print("\nFuselage and Cabin Information:")
print(tabulate(data_fuselage, headers=["Parameter", "Value"], tablefmt="grid"))

# Grouped data
print("\nWeights:")
print(tabulate(data_weights, headers=["Parameter", "Value"], tablefmt="grid"))

print("\nFuel Information:")
print(tabulate(data_fuel, headers=["Parameter", "Value"], tablefmt="grid"))

print("\nAerodynamics:")
print(tabulate(data_aerodynamics, headers=["Parameter", "Value"], tablefmt="grid"))

print("\nTank Information:")
print(tabulate(data_tank, headers=["Parameter", "Value"], tablefmt="grid"))
